# IBM Storage Archive EE health check program

-------------------------

# Introduction to check_spectrumarchive.sh
This utility performs status checks of IBM Storage Archive Enterprise Edition components. The components that can be checked are:
- status of software
- state of Storage Archive EE nodes
...
The utility uses the Storage Archive EE admin command line interface (eeadm) to obtain the status of the components in json format. The output of eeadm command is parsed and a decision is derived whether the status of the component is in `GOOD`, `WARNING` or `ERROR` state. This utility writes the results to standard out in one line including the status of the components and the return code is in accordance with the detected status. For components that are not in `GOOD` state further details are displayed. 

The utility is based on Storage Archive EE version 1.3 and above. It requires the `jq` tool to be installed on all Storage Archive EE nodes where this tool runs. 

The utility can be invoked to display the status of a single component or of all components. If all components are checked (option -e) then the utility can raise event notifications to the IBM Storage Scale event monitor. This requires the custom events to be configured. 
...
### Dependencies
This utility is based on Storage Archive EE version 1.3 and relies on the eeadm command. The EE admin command is specified in parameter `$EE_ADM_CMD` within the script. It has not been tested with the older ltfsee command. 

The tool `jq` is required. On RHEL it can be installed from the base repo using the command `dnf install jq`. The `jq` tool needs to be install on all nodes where `check_spectrumarchive.sh` is executed. 
For a satelite installation, the `jq` tool could be obtained from [stedolan.github.io/jq/](https://stedolan.github.io/jq/). Install the tool in path /usr/local/bin. The `jq` path can be changed within the script (paramater: `$JQ_TOOL`).


## Installation
Clone the git and transfer the `check_spectrumarchive.sh` utility to each Storage Archive EE node that needs to be monitored. Make the utility executable. 

Install the dependencies outline below. 

The utility can now be used from the command line or by an automated process. 

Optionally integrate the utility with an external monitoring tool such as Icinga. Or integrate the utility with the IBM Storage Scale event notifications.  
...
Options:
	-s			--> Verify IBM Storage Archive status
...
1. Integration with the IBM Storage Scale event monitoring framework
2. Integration with Icinga

These two options are further discussed below.


## Integration with IBM Storage Scale event monitoring

This utility can be invoked to check all components (`option -e`) and send events to the IBM Storage Scale monitoring framework in case WARNING or ERROR has been detected. Events will be send if the custom event notifications for this utility have been configured. 

IBM Storage Scale includes a monitoring framework that allows to send custom events. Custom events can be surfaced by the IBM Storage Scale GUI and send as event notifications through SNMP or email. 

To integrate this utility with the IBM Storage Scale event monitoring framework custom events need to be defined and configured. The file [custom.json](custom.json) included predefined custom events. The following events are pre-defined:
...
If the events are working then restart the Storage Scale GUI
...
Checking all componentes can be automated with the [IBM Storage Scale storage services automtation framework](https://github.com/nhaustein/StorageScaleAutomation). This framework can be configured to run the checks for all Storage Archive EE components periodically and raise events when appropriate. 



### Examples of events
Find below an example for the events raised to the Storage Scale system monitor:
...
If the Storage Scale GUI is installed and active the events raised by this utility when using option -e are propagated to the GUI event log. Event notifications can be configured to send events via email or SNMP. The component scope is NODE. 
...
The server is the Icinga server providing the graphical user interface and the option to configure monitored objects such as hostgroups, hosts and services. The hosts to be monitored are the Storage Archive nodes. The services are checked with the `check_spectrumarchive.sh` script. The Icinga server essentially calls the script on the remote Storage Archive nodes using NRPE. More information about Icinga: https://Icinga.com/products/


The client is the IBM Storage Archive nodes being monitored. The communication between the server and the client can be based on Nagios Remote Plugin Executor (NRPE). This requires to install and configure NRPE on the Storage Archive nodes. 
...
In order to monitor the Storage Archive nodes using NRPE the NRPE packages and optionally the nagios-plugins have to be installed and configured. These packages need to be installed on all Storage Archive to be monitored. 
...
Create the NRPE-configuration for the Storage Archive specific checks using this script. Note, the allowed_hosts must include the IP address of your Icinga server. Each check has a name given in [] which executes a particular command, such as `/usr/local/bin/check_spectrumarchive.sh -s`. Find an example below: 
...
First check that the Icinga server can communicate with the Storage Archive nodes using NRPE. For this purpose the check_NRPE plugin of the server can be used. The default location is: /usr/lib/nagios/plugins/check_NRPE. Find an example below:

```
/usr/lib/nagios/plugins/check_NRPE -H <IP of Storage Archive node>
```
...
```
/usr/lib/nagios/plugins/check_NRPE -H <IP of Storage Archive node> -c check_ee_state
```
...
<img src="https://github.com/nhaustein/check_Storagearchive/blob/master/images/icinga_ee2.JPG" width="868" alt="Icinga Storage Archive Integration">
